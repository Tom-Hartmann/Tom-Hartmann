...
async function checkValid() {
  if (!config.OWNER_ID) {
    console.log(chalk.bgYellowBright.black("[WARN] OWNER_ID_WAS_NOT_FOUND"));
  }
  if (!config.BOT_TOKEN) {
  ...
  if (!config.YT_COOKIE) {
    console.log(chalk.bgYellowBright.black("[WARN] YT_COOKIE_WAS_NOT_FOUND"));
  }
  if (!config.tenorAPI) {
    console.log(
      chalk.bgYellowBright.black("[WARN] TENOR_API_KEY_WAS_NOT_FOUND")
    );
  }
  if (!config.STEAM_API) {
    config.log(
      chalk.bgYellowBright.black("[WARN] STEAM_API_KEY_WAS_NOT_FOUND")
    );
  }
