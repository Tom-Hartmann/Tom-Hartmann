...
async function sendErrorLog(client, error, type) {
  try {
    ...
    // Add the request data field to the first embed
    let data = codeBlock(jsonString?.substring(0, 1020));
    if (data.length > 0) {
      embed.addField("Request data", data);
    }

    // Split the error stack into multiple chunks if necessary
    let stackChunks = stack.match(/[\s\S]{1,4000}/g) || [];
    for (let i = 0; i < stackChunks.length; i++) {
      // If this is not the first embed, create a new one
      if (i > 0) {
        embed = new DJS.EmbedBuilder().setColor(
          type === "error" ? "Red" : "Orange"
        );
      }

      // Add the error stack to the embed
      embed.setDescription(codeBlock(stackChunks[i]));
