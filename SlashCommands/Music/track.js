  ...
  run: async (client, interaction, args) => {
    ...
    if (interaction.options.getSubcommand() === "clearqueue") {
      ...
      if (!queue || !queue.playing)
        return client.say.errorMessage(
          interaction,
          "I’m currently not playing in this guild."
        );

      if (!client.utils.canModifyQueue(interaction)) return;

      if (queue.tracks.length < 1)
        return client.say.warnMessage(
          interaction,
          "There is currently no song in the queue."
        );
      ...
      if (!channel?.viewable)
        return client.say.warnMessage(
          interaction,
          "I need **`VIEW_CHANNEL`** permission."
        );
      if (!channel?.joinable)
        return client.say.warnMessage(
          interaction,
    ...
    if (channel?.full)
      ...
      let result = await client.player
        .search(string, { requestedBy: interaction.user })
        .catch(() => {});
      ...
      return client.say.infoMessage(
        interaction,
        "Backed to the previous song."
      );
      ...
      if (index) {
        ...
        if (
          !queue.tracks[songNum] ||
          songNum > queue.tracks.length ||
          songNum < 0
        )
          return client.say.errorMessage(
            interaction,
            "Provided Song Index does not exist."
          );
      ...
      const embed = client.say
        ...
        .setFooter(
          `Use \'\/loop <off|track|queue|autoplay>\' to change loop mode.`
        );
      ...
      if (fr === to)
        return client.say.warnMessage(
          interaction,
          "The song is already in this position."
        );
      ...
      return client.say.infoMessage(
        interaction,
        `**[${song.title}](${song.url})** has been moved to the **position ${to}** in the queue.`
      );
      ...
      if (queue.tracks.length < 1)
        return client.say.warnMessage(
          interaction,
          "There's no song to remove in the queue."
        );
      ...
      if (isNaN(timeString) && !timeString.includes(":"))
        return client.say.errorMessage(
          interaction,
          "Provide a valid duration to seek."
        );
      ...
      if (queue.tracks.length < 3)
        return client.say.warnMessage(
          interaction,
          "Need at least `3` songs in the queue to shuffle."
        );
      ...
      if (queue.tracks.length < 1 && queue.repeatMode !== 3)
        return client.say.warnMessage(
          interaction,
          "No more songs in the queue to skip."
        );
      ...
      if (!Number.isInteger(newVol) || newVol > 200 || newVol < 0)
        return client.say.warnMessage(
          interaction,
          "Provide a valid number between 1 to 200."
        );
      ...
      return client.say.infoMessage(
        interaction,
        `Volume is updated to \`${queue.volume}%\`.`
      );
