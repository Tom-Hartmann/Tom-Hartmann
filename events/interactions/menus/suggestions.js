...
module.exports = async (interaction, client) => {
  ...
  if (interaction.values[0] === "suggestions") {
    ...
    let data;
    try {
      data = await suggestData.findOne({
    ...
    if (!data) {
      msg.edit(
        "Please send the **CHANNEL ID** to be setup as sugesstions channel"
      );
      ...
      collector.on("collect", async (collected, returnValue) => {
        ...
        return msg.edit(
          `Suggestions channel is set to ${interaction.guild.channels.cache.get(
            channelID
          )}`
        );
      });
